...
Wanneer alle lagen waren toegevoegd werd het model gecompileerd en werden er nog 3 parameters meegegeven.
Een daarvan was de loss functie, dit is de functie die de loss berekend van bij het trainen van een model.
De loss is het aantal fouten die gemaakt worden bij het trainen, het type loss functie die gebruikt wordt hangt af van welk probleem er tracht opgelost te worden met het ontworpen neurale netwerk.
...
Een volgende parameter die gevraagd werd is de optimalisatie functie, deze functie zorgt ervoor dat de gemaakte loss bij een trainingsproces zo laag mogelijk is, dit doet het de leerbare parameters aan te passen in het neurale netwerk.\\
...
Vervolgens wordt een batch grootte meegegeven die bepaald hoeveel data het moet testen per keer, het aantal generaties en een splitsing van de data waarbij het opgesplitste deel gebruikt wordt voor validatie.
...
Wanneer het model zijn generaties had voltooid kon er gekeken worden naar de accuraatheid en de loss van elke generatie.
...
Uiteindelijk kijkt het gebruikte script naar de overeenkomende waarde met de lijst van schriftsystemen en heeft dit weer.
